    ...
    try {
      ...
      response.getIsikukaart().getValiskvalifikatsioonList().stream()
          ...
          .forEach(valisKval -> valineKvalArrayNode.addObject()
              .put("id", valisKval.isSetId() ? valisKval.getId() : null)
              .put("oppeasutuseNimi", valisKval.isSetOppeasutuseNimi()
                  ? valisKval.getOppeasutuseNimi() : null)
      ...
      response.getIsikukaart().getValiskvalifikatsioonList().stream()
          ...
          .forEach(enneKval -> enne2004KvalArrayNode.addObject()
              .put("id", enneKval.isSetId() ? enneKval.getId() : null)
              .put("oppeasutuseNimi", enneKval.isSetOppeasutuseNimi()
                  ? enneKval.getOppeasutuseNimi() : null)
