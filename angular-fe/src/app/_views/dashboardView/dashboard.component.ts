		...
		this.modalSubscription = this.http
			...
			.subscribe((response: any) => {
				if (response.error) {
					...
					/**
					 * Update user in auth service if names have changed
					 */
					if (response?.useEhisNames) {
						this.auth.userData.firstname = this.personalData?.eesnimi;
						this.auth.userData.lastname = this.personalData?.perenimi;
						this.auth.userData.usingEhisNames = true;
					}
				}
				this.loading = false;
