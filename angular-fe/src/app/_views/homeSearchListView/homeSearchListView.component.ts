...
export class HomeSearchListViewComponent {
	...
	public typesByLang: any = {
		et: [
			...
			{
				name: 'studyProgramme.label',
				sumLabel: 'study_programme',
				value: false,
				sum: 0,
			},
			{
				name: 'oska.future_job_opportunities',
				sumLabel: 'Oska',
				value: false,
				sum: 0,
			},
	...
	ngOnInit() {
		this.paramSubscription = this.route.queryParams.subscribe((params) => {
			...
			if (
				this.route.snapshot.queryParams['term'] !== this.param &&
				this.param.length
			) {
	...
	getContentLabel(contentType: string): string {
		return contentType.includes('oska')
			? 'oska.future_job_opportunities'
			: `${contentType}.label`;
	}
	...
	getResults(term, type) {
		...
		const path = this.settings.queryList('search', variables);
		this.dataSubscription = this.http.get(path).subscribe((data) => {
			this.updateParams('type', type.length ? type : null);
			this.results = this.filteredResults = data['entities'];

			this.types.forEach((type) => {
				type.sum = 0;
				type.value = this.typeArr.includes(type.sumLabel);
			});
			...
			this.filteredResults.forEach((res) => {
				this.types.forEach((type) => {
					if (
						type.sumLabel === 'Oska' &&
						this.oskaTypes.includes(res.content_type)
					) {
						type.sum += 1;
					}
					if (type.sumLabel === res.content_type) {
						type.sum += 1;
					}
				});
			});
			...
			this.filteredResults = this.results.filter((res) => {
				return (
					this.typeArr.includes(res.content_type) ||
					(this.typeArr.includes('Oska') &&
						this.oskaTypes.includes(res.content_type))
				);
			});

			this.filteredResults = !this.typeArr.length
				? this.results
	...
	loadMore() {
		...
		this.listLimit =
			listLimit + listStep < listLength ? listLimit + listStep : listLength;
	...
	constructCrumbs() {
		...
		if (this.route.snapshot.queryParams['term']) {
			...
			crumbUrl = `${translatePath('/otsing')}?term=${
				this.route.snapshot.queryParams['term']
			}`;
		} else {
	...
	filterView(id) {
		...
		this.types.forEach((type) => {
			if (type.value) {
				this.typeArr.push(type.sumLabel);
			}
		});
	...
	populateSuggestionList(searchText, debounceTime) {
		...
		this.debouncer = setTimeout((_) => {
			...
			this.suggestionSubscription = this.http
				.get('testAutocomplete', { params: variables })
				.subscribe((res) => {
					this.autocompleteLoader = false;
					this.suggestionList =
						res['data']['CustomElasticAutocompleteQuery'] || [];
					this.suggestionSubscription.unsubscribe();
				});
		}, debounceTime);
	}
