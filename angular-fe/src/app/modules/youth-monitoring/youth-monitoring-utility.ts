...
export class YouthMonitoringUtility {
	...
	public static mapDetail(
		input: YouthMonitoringDetailTabDto
	): MappedYouthMonitoringDetailTab[] {
		const tabs = input.fieldYouthMonitorTab.map((item) => item.entity);
		return tabs.map((tab) => {
			const {
				fieldYouthMonitorTabPage: { entity: data },
				fieldAccordionIcon,
				fieldAccordionTitle,
			} = tab;
			return {
				...
				fieldYouthMonitorTabPage: {
					...
					images: data.fieldYouthGallery
						?.filter((item) => item.entity.fieldMediaImg)
						.map((item) => this.mapImage(item.entity.fieldMediaImg)),
					videos: data.fieldYouthGallery
						?.filter((item) => item.entity.fieldMediaVid)
						.map((item) => item.entity.fieldMediaVid),
								...
								.map((accordion) => ({
									entity: {
										...
										fieldYouthPicture: accordion.entity.fieldYouthPicture
											...
											: null,
										fieldYouthLink: this.mapLink(
											accordion.entity.fieldYouthLink
										),
									},
								}))
						: null,
	...
	public static mapImages(
		input: YouthMonitoringPicture | YouthMonitoringPicture[]
	) {
	...
	public static sortTitle<T extends { title?: string }>(links: T[]): T[] {
		return links.sort((a, b) =>
			a.title
				.toString()
				.localeCompare(b.title.toString(), 'et', { numeric: true })
		);
	}
