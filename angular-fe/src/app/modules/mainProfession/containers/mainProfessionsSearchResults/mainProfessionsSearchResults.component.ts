...
export class MainProfessionsSearchResultsComponent implements OnDestroy {
	...
	private getValue(value, key) {
		let tmpValue = value;
		tmpValue =
			likeFields.indexOf(key) !== -1 ? (value ? `%25${value}%25` : '') : value;
		return tmpValue;
	}
	...
	private parseValues(queryParams) {
		...
		Object.keys(tmpParams).forEach((item) => {
			...
			if (searchResultKeys[item]) {
				...
				values[searchResultKeys[item].key] = this.getValue(
					tmpParams[item],
					item
				).replace(/\;/gim, ',');
			} else {
			...
			if (values[item] === '') {
				values[item] =
					defaultValues[item] || defaultValues[item] === 0
						? defaultValues[item]
						: '';
			}
	...
	private getData(values, append?: boolean, listValue?: Object[]): void {
		...
		this.getDataDebounce = setTimeout(() => {
			...
			this.searchWithParams = !!Object.values(
			...
			if (listValue) {
				...
				this.nonProfessionCount =
					this.scrollRestorationValues[this.type].nonProfessionCount;
				this.professionCount =
					this.scrollRestorationValues[this.type].professionCount;
				this.listCount = this.scrollRestorationValues[this.type].listItemCount;
				...
				this.httpWatcher = this.http.get(path).subscribe({
					next: (response) => {
						...
						// 	this.nonProfessionCount =
						// 		response['data']['countProfessionsFalseValue']['count'];
						// 	this.professionCount =
						// 		response['data']['countProfessionsValue']['count'];
						// }
						this.listCount = Number(response['count']);
						const listData = response['entities'];
						if (append) {
	...
	public selectArbitraryHighlightedJob(): void {
		if (this.list && this.list.length && !this.searchWithParams) {
			...
			if (filteredList.length) {
				...
				const jobSubscription = this.http
					.get(
						this.settings.query('oskaMainProfessionDetailView', {
							path: initialFilteredJobPath,
						})
					)
					.pipe(take(1))
					.subscribe({
						next: (response) => {
							...
							if (!filteredJob['fixedLabel']) {
								filteredJob['fixedLabel'] = {
									entity: {
										entityLabel:
											this.competitionLabels[
												filteredJob['fieldFillingBar'] - 1
											],
									},
								};
							}
							filteredJob['fieldPictogram'] =
								response['data']['route']['entity']['fieldPictogram'];
