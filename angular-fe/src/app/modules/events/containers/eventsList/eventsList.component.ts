...
export class EventsConfig {
	...
	getApollo(language) {
		const obArr = {
			...
			titleValue: this.titleValue ? `%${this.titleValue}%` : '',
...
export class EventsListComponent extends FiltersService implements OnInit {
	...
	constructor(
		...
		super(null, null);
		let subscription = router.events
			.pipe(filter((event) => event instanceof NavigationEnd))
			.subscribe((event: NavigationEnd) => {
				// (/^\/sündmused\/kalender/g).test(decodeURI(event.url))
				if (
					decodeURI(event.url) === translatePath('/sündmused/kalender') &&
	...
	getMonthName(month: any) {
		return moment('2018-' + month + '-01', 'YYYY-M-DD')
			.format('MMMM')
			.toLowerCase();
	}
	...
	getDayName(year, month: any, day: any, isUnix: boolean = false) {
		...
		if (isUnix) {
			...
			return moment(year + '-' + month + '-' + day, 'YYYY-M-DD')
				.format('dddd')
				.toLowerCase();
		}
	}
	...
	generateCalendar(urlDate: boolean = false) {
		if (this.filterFormItems.dateFrom && urlDate) {
			this.month = moment(this.filterFormItems.dateFrom, 'DD-MM-YYYY').format(
				'M'
			);
		} else {
		...
		this.monthName = moment(this.year + '/' + this.month, 'YYYY/M').format(
			'MMMM'
		);
	...
	changeView(view: any, update: boolean = true) {
		...
		switch (view) {
			case 'calendar':
				...
				this.router.navigate([translatePath('/sündmused/kalender')], {
					queryParamsHandling: 'preserve',
				});
				break;
			case 'list':
				this.router.navigate([translatePath('/sündmused')], {
					queryParamsHandling: 'preserve',
				});
			default:
				break;
	...
	loadMore() {
		...
		let subscriber = this.http.get(path).subscribe((response) => {
			...
			this.eventListRaw = this.eventListRaw.concat(response['entities']);
			...
			this.scrollRestoration.restorationValues.next({
				...
				eventsList: {
					values: this.params,
					list: this.eventListRaw,
					canLoadMore: !this.listEnd,
				},
			});
	...
	ngOnInit() {
		...
		this.route.params.subscribe((params: ActivatedRoute) => {
		...
		if (
			decodeURI(this.path) === translatePath('/sündmused/kalender') &&
			window.innerWidth > 1024
		) {
		...
		this.route.queryParams.subscribe((params) => {
			...
			Object.keys(params).forEach((i) => {
				tmpParams[i] = params[i].replace(/\;/gim, ',');
			});
			if (Object.keys(params).length > 0) {
				this.queryString =
					'?' +
					Object.keys(tmpParams)
						.reduce((all, current) => {
		...
		Object.keys(this.params).forEach((i) => {
			tmpParams[i] = this.params[i].replace(/\;/gim, ',');
		});
	...
	organizeList(data: any) {
		...
		for (var i in list) {
			...
			let earliest = this.dateToStamp(entry['fieldEventMainDate']);
			let timeEarliest;

			entry['firstEventTime'] = entry['fieldEventStartTimeMain'];
			entry['firstEventUnix'] = this.dateToStamp(entry['fieldEventMainDate']);
			...
			if (!tmpList[year][month]) {
				tmpList[year][month] = {};
			}
			if (!tmpList[year][month][day]) {
	...
	typesDropdownSort(e) {
		if (!e) {
			...
			if (this.filterFormItems.types) {
				const sortedSelected = this.filterFormItems.types.sort(
					(a: any, b: any) => {
			...
			let otherValues = this.eventsTypes
				.filter((type) => {
	...
	tagsDropdownSort(e) {
		if (!e) {
			...
			if (this.filterFormItems.tags) {
				const sortedSelected = this.eventsTags
					.filter((tag) =>
						this.filterFormItems.tags.find(
							(selectedTag) => tag.entityId === selectedTag
						)
					)
					.sort((a: any, b: any) => {
			...
			const otherValues = this.eventsTags
				.filter((e) => !selected.find((x) => x.entityId === e.entityId))
				.sort((a: any, b: any) => {
					if (a.entityLabel.toUpperCase() > b.entityLabel.toUpperCase()) {
						return 1;
					}
	...
	getData(listValues?: any) {
		this.loadFlag = true;
		if (!this.filterFull) {
			this.filterFull = this.filterFullProperties.some(
				(property) => this.params[property] !== undefined
			);
		}
		...
		if (
			...
			moment(this.params['dateFrom'], 'DD-MM-YYYY').isValid()
		) {
			this.eventsConfig.dateFrom = moment(this.params['dateFrom'], 'DD-MM-YYYY')
				.format('YYYY-MM-DD')
				.toString();
		} else {
		...
		if (
			this.params['dateTo'] &&
			moment(this.params['dateTo'], 'DD-MM-YYYY').isValid()
		) {
			this.eventsConfig.dateTo = moment(this.params['dateTo'], 'DD-MM-YYYY')
				.format('YYYY-MM-DD')
				.toString();
		}

		if (this.view == 'calendar') {
			this.eventsConfig.dateFrom = moment(
				...
				.format('YYYY-MM-DD')
				.toString();
			this.eventsConfig.dateTo = moment(
				'01-' + this.month + '-' + this.year,
				'DD-MM-YYYY'
			)
				.endOf('month')
        ...
        */
		...
		if (this.view == 'list') {
			...
			let startTimeSeconds: any =
				startTimeHours * 60 * 60 + startTimeMinutes * 60;
			...
			this.dataSubscription = this.http.get(path).subscribe({
				next: (data) => {
					...
					if (this.view == 'list') {
						...
						this.eventListRaw = data['entities'];

						this.eventList = this.organizeList(this.eventListRaw);
						// if (data['entities'] && (data['entities'].length < this.eventsConfig.limit)){
						//   this.listEnd = true;
						// }
						if (this.eventListRaw && this.eventListRaw.length === this.count) {
							this.listEnd = true;
						...
						this.dataToCalendar(JSON.stringify(data['entities']));
					}
	...
	private dateToStamp(date: string) {
