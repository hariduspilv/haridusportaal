...
import { Content, Entity } from '@app/_core/models/interfaces/main';
...
export class StudyUtility {
	private static joinArrayToString(
		elements: (string | number | Content)[] | string
	): string {
		if (!Array.isArray(elements)) return elements;
		if (elements.some((entry) => !!entry['value'])) {
			elements = (elements as Content[]).map(({ value }) => value);
		}
		return elements?.join(', ') || '';
	}

	private static mapOptionTypesToFormItemOptions(
		options: Entity[]
	): FormItemOption[] {
	...
	private static gatherJoinedInlineFields(study: Study): string[] {
		const flattenedFieldPublicationTypes =
			study.fieldRightColumn?.[0]?.fieldStudy?.[0]?.fieldPublicationType?.map(
				(publication) => publication || ''
			);
		return [
			this.joinArrayToString(
				study.fieldRightColumn?.[0]?.fieldStudy?.[0]?.fieldAuthor
			),
			this.joinArrayToString(
				study.fieldRightColumn?.[0]?.fieldStudy?.[0]?.fieldYear
			),
			this.joinArrayToString(flattenedFieldPublicationTypes),
		];
	}

	private static gatherJoinedRightColumnDataFields(
		studyData: StudyPageFieldRightColumnDataEntity
	): MappedStudyPageFieldRightColumnStudyData {
		const flattenedFieldPublicationTypes = studyData.fieldPublicationType?.map(
			(type) => type.entity.entityLabel
		);
		const flattenedFieldPublicationLanguages =
			studyData.fieldPublicationLang?.map(
				(publication) => publication.entity.entityLabel
			);
		return {
			fieldAuthor: this.joinArrayToString(studyData?.fieldAuthor || []),
			fieldAuthorInstitution: this.joinArrayToString(
				studyData?.fieldAuthorInstitution || []
			),
			fieldPublicationLang: this.joinArrayToString(
				flattenedFieldPublicationLanguages || []
			),
			...
			fieldPublicationType: this.joinArrayToString(
				flattenedFieldPublicationTypes || []
			),
		};
	}
	...
	public static mapStudyListViewEntities(entities: Study[]): MappedStudy[] {
		return entities.map((study: Study) => {
			const mappedInlineFields = this.gatherJoinedInlineFields(study).filter(
				(field: string) => field
			);
			return {
				...study,
	...
	public static flattenStudyListFilterOptions(
		filterResponse: StudyListViewFilterQueryResponse
	): MappedStudyFilters {
		return {
	...
	public static joinResponseWithPreviousValues(
		list: MappedStudy[],
		studyListResponseEntities: Study[],
		loadMoreContent?: boolean
	): MappedStudy[] {
		const mappedStudyListElements = StudyUtility.mapStudyListViewEntities(
			studyListResponseEntities
		);
	...
	public static studyListMappedData(
		...
		return this.joinResponseWithPreviousValues(
			list,
			studyListResponseEntities,
			loadMoreContent
		);
	...
	public static mapStudySidebarBlockData(
		sidebarData: StudyPageFieldRightColumn
	): MappedStudyPageFieldRightColumn {
		return {
			fieldLinks: sidebarData?.entity?.fieldAdditionalLinks?.entity?.fieldLinks,
			fieldStudy: this.gatherJoinedRightColumnDataFields(
				sidebarData?.entity?.fieldStudy?.entity
			),
		};
	}
