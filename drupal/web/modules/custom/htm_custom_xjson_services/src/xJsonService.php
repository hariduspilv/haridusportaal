...
class xJsonService implements xJsonServiceInterface {
  ...
  private function createVocationResponse($data) {
    ...
    if($data['body']['steps'][$data['header']['current_step']]['sequence'] === 1) {
      ...
      $current_step_value = $xjson_definition['body']['steps'][$keys[$data['body']['steps'][$data['header']['current_step']]['sequence'] - 1]]['data_elements'];
      ...
      if($proof_file_is_required) {
        switch(strtolower($proof_file_is_required)) {
          case 'portfoolio':
   ...
   * @throws \Drupal\Component\Plugin\Exception\PluginNotFoundException
   */
  public function getEntityChangedFormName ($form_name)
  {
    $entityStorage = \Drupal::entityTypeManager()->getStorage('x_json_entity');
    $connection = \Drupal::database();
    $query = $connection->query("SELECT id FROM x_json_entity WHERE xjson_definition->'header'->>'form_name' = :id ", [':id' => $form_name]);
    ...
    switch ($element_type) {
      ...
      case 'date':
        ...
        if (isset($element['default_value'])) {
          $element['value'] = $element['default_value'] === 'today' ? date('d.m.Y') : $element['default_value'];
        }
        break;
      case 'number':
        if ($table) $additional_keys = ['width', 'min', 'max'];
    ...
    foreach($xml_data as $xml_item) {
      ...
      if($name === strtolower($value)) {
        if(in_array(((Array)$xml_item->eelnõue)[0], $required)) {
          return ((Array)$xml_item->eelnõue)[0];
        }
      }
    }
    return false;
