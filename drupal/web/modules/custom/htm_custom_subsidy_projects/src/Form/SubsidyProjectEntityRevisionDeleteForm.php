...
/**
 * Provides a form for deleting a Subsidy project revision.
 *
 * @ingroup htm_custom_subsidy_projects
 */
class SubsidyProjectEntityRevisionDeleteForm extends ConfirmFormBase {
  ...
  /**
   * Constructs a new SubsidyProjectEntityRevisionDeleteForm.
   *
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_manager->getStorage('subsidy_project_entity'),
  ...
  public function getFormId() {
    return 'subsidy_project_entity_revision_delete_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($this->connection->query('SELECT COUNT(DISTINCT vid) FROM {subsidy_project_entity_field_revision} WHERE id = :id', [':id' => $this->revision->id()])->fetchField() > 1) {
      $form_state->setRedirect(
