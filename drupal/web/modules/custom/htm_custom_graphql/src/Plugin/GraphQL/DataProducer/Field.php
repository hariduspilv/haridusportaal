 ...
 *     ),
 *     "field_value" = @ContextDefinition("string",
 *       label = @Translation("Field value field")
 *     ),
 *
 *   }
 * )
...
class Field extends DataProducerPluginBase {
   ...
   */
  public function resolve(EntityInterface $entity, string $field, string $field_value): ?string {
    if (!$entity instanceof FieldableEntityInterface || !$entity->hasField($field)) {
      return NULL;
    }
    $value = $entity->get($field)->{$field_value};
