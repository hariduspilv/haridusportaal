 ...
 */
class OskaInfographImportDataForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $batch = [
      'title' => t('Processing Oska Infograph data ....--'),
      'operations' => [
        [
          '\Drupal\htm_custom_oska\ProcessOskaInfographData::ValidateFile',
          [$filename, $file_array]
        ],
        [
          '\Drupal\htm_custom_oska\ProcessOskaInfographData::CreateOskaGraphFilters',
          [$filename, $file_array]
        ]
      ],
      'error_message' => t('The migration process has encountered an error.'),
      'finished' => '\Drupal\htm_custom_oska\ProcessOskaInfographData::ProcessOskaInfographDataFinishedCallback'
    ];

    batch_set($batch);
    Cache::invalidateTags([$filename.'_csv']);
    $form_state->setRedirect('htm_custom_oska.oska_infograph_csv_list_form');
  }
