...
(function ($) {
    ...
    ClockPicker.prototype.show = function (e) {
        ...
        setTimeout(function () {
            // Not show again
            if (clockpicker.isShown) {
                return;
            }
            raiseCallback(clockpicker.options.beforeShow);
            $(':input').each(function () {
                $(clockpicker).attr('tabindex', -1);
            });
            var self = clockpicker;
            // Initialize
            clockpicker.input.blur();
            clockpicker.popover.addClass('picker--opened');
            clockpicker.input.addClass('picker__input picker__input--active');
            $(document.body).css('overflow', 'hidden');
            // Get the time
            var value = ((clockpicker.input.prop('value') || clockpicker.options['default'] || '') + '').split(':');
            if (clockpicker.options.twelvehour && !(typeof value[1] === 'undefined')) {
                if (value[1].indexOf("AM") > 0) {
                    clockpicker.amOrPm = 'AM';
                } else {
                    clockpicker.amOrPm = 'PM';
                }
                value[1] = value[1].replace("AM", "").replace("PM", "");
            }
            if (value[0] === 'now') {
                var now = new Date(+new Date() + clockpicker.options.fromnow);
                value = [now.getHours(), now.getMinutes()];
                if (clockpicker.options.twelvehour) {
                    clockpicker.amOrPm = value[0] >= 12 && value[0] < 24 ? 'PM' : 'AM';
                }
            }
            clockpicker.hours = +value[0] || 0;
            clockpicker.minutes = +value[1] || 0;
            clockpicker.spanHours.html(clockpicker.hours);
            clockpicker.spanMinutes.html(leadingZero(clockpicker.minutes));
            if (!clockpicker.isAppended) {

                // Append popover to input by default
                var containerEl = document.querySelector(clockpicker.options.container);
                if (clockpicker.options.container && containerEl) {
                    containerEl.appendChild(clockpicker.popover[0]);
                } else {
                    clockpicker.popover.insertAfter(clockpicker.input);
                }

                if (clockpicker.options.twelvehour) {
                    if (clockpicker.amOrPm === 'PM') {
                        clockpicker.spanAmPm.children('#click-pm').addClass("text-primary");
                        clockpicker.spanAmPm.children('#click-am').removeClass("text-primary");
                    } else {
                        clockpicker.spanAmPm.children('#click-am').addClass("text-primary");
                        clockpicker.spanAmPm.children('#click-pm').removeClass("text-primary");
                    }
                }
                // Reset position when resize
                $win.on('resize.clockpicker' + clockpicker.id, function () {
                    if (self.isShown) {
                        self.locate();
                    }
                });
                clockpicker.isAppended = true;
            }
            // Toggle to hours view
            clockpicker.toggleView('hours');
            // Set position
            clockpicker.locate();
            clockpicker.isShown = true;
            // Hide when clicking or tabbing on any element except the clock and input
            $doc.on('click.clockpicker.' + clockpicker.id + ' focusin.clockpicker.' + clockpicker.id, function (e) {
                var target = $(e.target);
            ...
            $doc.on('keyup.clockpicker.' + clockpicker.id, function (e) {
                if (e.keyCode === 27) {
                    self.hide();
                }
            });
            raiseCallback(clockpicker.options.afterShow);
        }, 200);
    };
    // Hide popover
