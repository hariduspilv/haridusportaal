...
class xJsonRestResource extends ResourceBase {
	...
	private function returnRighstDzeison ($data) {
        ...
        \Drupal::logger('xjson')->notice(json_encode($request_body));
        \Drupal::logger('xjson')->notice(json_encode($response));
	...
	private function returnBuildedResponse ($response) {
		...
		dump($builded_response);
		$builded_response['messages'] = json_decode("{}");
		$builded_response['body']['messages'] = json_decode("{}");
