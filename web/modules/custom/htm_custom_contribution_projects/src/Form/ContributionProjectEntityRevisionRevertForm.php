...
/**
 * Provides a form for reverting a Contribution project revision.
 *
 * @ingroup htm_custom_contribution_projects
 */
class ContributionProjectEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  /**
   * Constructs a new ContributionProjectEntityRevisionRevertForm.
   *
  ...
  public function getFormId() {
    return 'contribution_project_entity_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Contribution project: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    \Drupal::messenger()->addMessage(t('Contribution project %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\htm_custom_contribution_projects\Entity\ContributionProjectEntityInterface $revision
